...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params; const business = await getPublicBusiness(slug);
  return business ? { title: `${business.name} — Agende seu horário`, description: `Escolha serviço, profissional, data e horário para agendar online em ${business.name}.` } : {};
}
...
export default async function PublicBusinessPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="public-shell"><nav className="public-nav"><NexoBrand /><a href={`https://wa.me/${business.phone.replace(/\D/g, "")}`} target="_blank" rel="noreferrer">Falar no WhatsApp</a></nav>
    ...
    <section className="public-section"><div className="section-title"><div><p className="eyebrow">Equipe</p><h2>Profissionais</h2></div></div><div className="public-grid">{business.professionals.map((professional) => <article className="public-card" key={professional.id}><span className="public-avatar">{professional.name.slice(0, 1)}</span><h3>{professional.name}</h3><p>{professional.bio || "Profissional da equipe."}</p></article>)}</div></section><footer className="public-footer">Powered by <strong>NEXO Agenda</strong></footer>
  </main>;
}
