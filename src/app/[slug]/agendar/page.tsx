...
export default async function BookingPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="booking-shell"><nav className="public-nav"><Link href={`/${business.slug}`} className="booking-back">← {business.name}</Link><NexoBrand compact /><span>Agendamento seguro</span></nav><header className="booking-header"><p className="eyebrow">Reserve seu horário</p><h1>Vamos encontrar o melhor momento.</h1><p>Não é necessário criar uma conta.</p></header><BookingFlow business={business} /></main>;
}
